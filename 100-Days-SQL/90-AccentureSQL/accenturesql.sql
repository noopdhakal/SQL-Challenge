...
MODIFY email_id VARCHAR(15) COLLATE SQL_Latin1_General_CP1_CS_AS;
...
with cte as (
    select emp.*
    ,ASCII(emp.email_id) as ascii_email
    , row_number() over(partition by emp.email_id order by ASCII(emp.email_id) desc) as rn
    from employees emp
)
select * from cte where rn=1;

SELECT LOWER(email_id),
       COUNT(*)
FROM employees
